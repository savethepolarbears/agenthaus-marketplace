...
if printf '%s\n' "$INPUT" | grep -qiE 'wp\s+(core\s+update)' && ! printf '%s\n' "$INPUT" | grep -qiE '--dry-run'; then
...
if printf '%s\n' "$INPUT" | grep -qiE 'wp\s+db\s+(drop|reset|import)'; then
...
if printf '%s\n' "$INPUT" | grep -qiE 'wp\s+search-replace' && ! printf '%s\n' "$INPUT" | grep -qiE '--dry-run'; then
